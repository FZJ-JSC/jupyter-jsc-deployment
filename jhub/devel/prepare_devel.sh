...
K8SMGRHDFCLOUD_AUTHENTICATION_TOKEN=$(echo -n "jupyterhub:${JUPYTERHUB_USER_PASS}" | base64 -w 0)

JUPYTERHUB_USER_PASS=$(skj get secret drf-unicoremgr-passwds -o jsonpath='{.data.JUPYTERHUB_USER_PASS}' | base64 -d)
UNICOREMGR_AUTHENTICATION_TOKEN=$(echo -n "jupyterhub:${JUPYTERHUB_USER_PASS}" | base64 -w 0)

JUPYTERHUB_USER_PASS=$(skj get secret drf-tunnel-passwds -o jsonpath='{.data.JUPYTERHUB_USER_PASS}' | base64 -d)
TUNNEL_AUTHENTICATION_TOKEN=$(echo -n "jupyterhub:${JUPYTERHUB_USER_PASS}" | base64 -w 0)

OAUTH_CLIENT_ID=$(skj get secret hub-custom -o jsonpath='{.data.oauth_client_id_password}' | base64 -d)
OAUTH_CLIENT_SECRET_TMP=$(skj get secret hub-custom -o jsonpath='{.data.oauth_client_secret_password}' | base64 -d)
OAUTH_CLIENT_SECRET=${OAUTH_CLIENT_SECRET_TMP//\"/\\\"}
